...
impl HospitalContract {
    ...
    pub fn register_patient(
        ...
        // Initialize empty test list for patient tests
    ...
    pub fn register_doctor(
        ...
        // use turbofish to initalize a new Vec of u64 DoctorTests
        env.storage()
            .instance()
    ...
    pub fn get_medical_test(env: Env, test_id: u64) -> MedicalTest {
        ...
        if !env.storage().instance().has(&DataKey::MedicalTest(test_id)) {
            panic!("Medical test not found");
        }

        // Retrieve and return the test
        env.storage()
            .instance()
            .get(&DataKey::MedicalTest(test_id))
            .unwrap()
    }
    ...
    pub fn record_medical_test(
        ...
        env.storage()
            .instance()
            .set(&DataKey::MedicalTest(new_test_id), &test);

        // Update the global test count
        env.storage()
            .instance()
            .set(&DataKey::TestCount, &new_test_id);

        // Update PatientTests list
    ...
    pub fn get_tests_for_patient(env: Env, patient_id: u64) -> Vec<MedicalTest> {
        // Get list of test IDs for this patient
        let test_ids: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::PatientTests(patient_id))
            .unwrap_or_else(|| panic!("No tests found for this patient"));

        // Create a Tests Vec for the results
        let mut tests = Vec::new(&env);

        // Push each MedicalTest into the Tests Vec
        for id in test_ids {
            let test = env
                .storage()
                .instance()
    ...
    pub fn get_tests_for_doctor(env: Env, doctor_id: u64) -> Vec<MedicalTest> {
        // Get list of test IDs for this doctor
        let test_ids: Vec<u64> = env
            .storage()
            .instance()
            .get(&DataKey::DoctorTests(doctor_id))
            .unwrap_or_else(|| panic!("No tests found for this doctor"));

        // Create a tests vec for the results
        let mut tests: Vec<MedicalTest> = Vec::new(&env);
    ...
    pub fn list_all_medical_tests(env: Env) -> Vec<MedicalTest> {
        // Get total number of tests recorded
        let test_count: u64 = env
            .storage()
            .instance()
            .get(&DataKey::TestCount)
        ...
        // Loop through all test IDs from 1 to test_count
        for id in 1..=test_count {
            let test = env
                .storage()
                .instance()
                .get(&DataKey::MedicalTest(id))
                .expect("Test not found");
            tests.push_front(test);
        }
        tests
    }
}
